...
class WC_Wallee_Migration {
	...
	const WALLEE_CK_DB_VERSION = 'wc_wallee_db_version';
	 ...
	 */
	const WALLEE_DEPRECATED_TABLE_PREFIX = 'wc_';
	const WALLEE_DEPRECATED_PLUGIN_PREFIX = 'woo-';
	...
	private static function check_requirements() {
		...
		if ( version_compare( PHP_VERSION, WC_WALLEE_REQUIRED_PHP_VERSION, '<' ) ) {
			$errors[] = sprintf( esc_html__( "PHP %1\$s+ is required. (You're running version %2\$s)", 'woo-wallee' ), WC_WALLEE_REQUIRED_PHP_VERSION, PHP_VERSION );
		}
		if ( version_compare( $wp_version, WC_WALLEE_REQUIRED_WP_VERSION, '<' ) ) {
			$errors[] = sprintf( esc_html__( "WordPress %1\$s+ is required. (You're running version %2\$s)", 'woo-wallee' ), WC_WALLEE_REQUIRED_WP_VERSION, $wp_version );

		}

		if ( ! is_plugin_active( 'woocommerce/woocommerce.php' ) ) {
			/* translators: %s+: version */
			$errors[] = sprintf( esc_html__( 'Woocommerce %s+ has to be active.', 'woo-wallee' ), WC_WALLEE_REQUIRED_WC_VERSION );
		} else {
			...
			if ( version_compare( $woocommerce_data['Version'], WC_WALLEE_REQUIRED_WC_VERSION, '<' ) ) {
				$errors[] = sprintf( esc_html__( "Woocommerce %1\$s+ is required. (You're running version %2\$s)", 'woo-wallee' ), WC_WALLEE_REQUIRED_WC_VERSION, $woocommerce_data['Version'] );
			}
		}
		...
		try {
			...
			$errors[] = __( "Install the PHP cUrl extension or ensure the 'stream_socket_client' function is available.", 'woo-wallee' );
		}
		...
		if ( ! empty( $errors ) ) {
			...
			wp_die(
				sprintf(
					...
					esc_html__( 'Please check the following requirements before activating: %s', 'woo-wallee' ),
					esc_html( $error_list )
				),
				esc_html__( 'Could not activate plugin wallee.', 'woo-wallee' ),
	...
	private static function migrate_db() {
		$current_version = get_option( self::WALLEE_CK_DB_VERSION, 0 );
		foreach ( self::$db_migrations as $version => $function_name ) {
			if ( version_compare( $current_version, $version, '<' ) ) {
				...
				update_option( self::WALLEE_CK_DB_VERSION, $version );
	...
	public static function wpmu_drop_tables( $tables ) {
		...
		$tables[] = $wpdb->prefix . 'wallee_method_config';
		$tables[] = $wpdb->prefix . 'wallee_transaction_info';
		$tables[] = $wpdb->prefix . 'wallee_token_info';
		$tables[] = $wpdb->prefix . 'wallee_completion_job';
		$tables[] = $wpdb->prefix . 'wallee_void_job';
		$tables[] = $wpdb->prefix . 'wallee_refund_job';
	...
	public static function plugin_row_meta( $links, $file ) {
		if ( WC_WALLEE_PLUGIN_BASENAME === $file ) {
			$row_meta = array(
				'docs' => '<a href="https://plugin-documentation.wallee.com/wallee-payment/woocommerce/3.3.20/docs/en/documentation.html" aria-label="' . esc_html__( 'View Documentation', 'woo-wallee' ) . '">' . esc_html__( 'Documentation', 'woo-wallee' ) . '</a>',
			);
	...
	public static function update_1_0_0_initialize() {
		...
		$table_method_config = $wpdb->prefix . 'wallee_method_config';
		...
		$table_token_info = $wpdb->prefix . 'wallee_token_info';
		...
		$table_completion_job = $wpdb->prefix . 'wallee_completion_job';
		...
		$table_void_job = $wpdb->prefix . 'wallee_void_job';
		...
		$table_refund_job = $wpdb->prefix . 'wallee_refund_job';
	...
	public static function update_1_0_3_image_domain() {
		...
		$table_transaction_info = $wpdb->prefix . 'wallee_transaction_info'; //phpcs:ignore
		$table_method_config = $wpdb->prefix . 'wallee_method_config'; //phpcs:ignore
	...
	public static function update_1_0_4_failure_msg_and_attribute() {
		...
		$table_transaction_info = $wpdb->prefix . 'wallee_transaction_info'; //phpcs:ignore
		$table_attribute_options = $wpdb->prefix . 'wallee_attribute_options'; //phpcs:ignore
	...
	public static function update_1_0_6_shorten_table_names() {
		...
		$table_prefix = self::WALLEE_DEPRECATED_TABLE_PREFIX;
		$table_rename_map = array(
			"{$wpdb->prefix}{$table_prefix}woocommerce_wallee_attribute_options" => "{$wpdb->prefix}{$table_prefix}wallee_attribute_options",
			"{$wpdb->prefix}{$table_prefix}woocommerce_wallee_completion_job" => "{$wpdb->prefix}{$table_prefix}wallee_completion_job",
			"{$wpdb->prefix}{$table_prefix}woocommerce_wallee_method_configuration" => "{$wpdb->prefix}{$table_prefix}wallee_method_config",
			"{$wpdb->prefix}{$table_prefix}woocommerce_wallee_refund_job" => "{$wpdb->prefix}{$table_prefix}wallee_refund_job",
			"{$wpdb->prefix}{$table_prefix}woocommerce_wallee_token_info" => "{$wpdb->prefix}{$table_prefix}wallee_token_info",
			"{$wpdb->prefix}{$table_prefix}woocommerce_wallee_transaction_info" => "{$wpdb->prefix}{$table_prefix}wallee_transaction_info",
			"{$wpdb->prefix}{$table_prefix}woocommerce_wallee_void_job" => "{$wpdb->prefix}{$table_prefix}wallee_void_job",
		);
	...
	public static function update_1_0_7_migrate_plugin_name_and_tables() {
		...
		$table_prefix = self::WALLEE_DEPRECATED_TABLE_PREFIX;
		$table_rename_map = array(
			"{$wpdb->prefix}{$table_prefix}wallee_attribute_options" => "{$wpdb->prefix}wallee_attribute_options",
			"{$wpdb->prefix}{$table_prefix}wallee_completion_job" => "{$wpdb->prefix}wallee_completion_job",
			"{$wpdb->prefix}{$table_prefix}wallee_method_config" => "{$wpdb->prefix}wallee_method_config",
			"{$wpdb->prefix}{$table_prefix}wallee_refund_job" => "{$wpdb->prefix}wallee_refund_job",
			"{$wpdb->prefix}{$table_prefix}wallee_token_info" => "{$wpdb->prefix}wallee_token_info",
			"{$wpdb->prefix}{$table_prefix}wallee_transaction_info" => "{$wpdb->prefix}wallee_transaction_info",
			"{$wpdb->prefix}{$table_prefix}wallee_void_job" => "{$wpdb->prefix}wallee_void_job",
		);
		self::rename_tables( $table_rename_map );
	}
			...
			add_action(
				...
				function () {
					...
					<div class="notice notice-error">
						<p><?php __( 'WooCommerce is not activated. Please activate WooCommerce to use the payment integration.', 'woo-wallee' ); ?></p>
					</div>
					<?php
				}
			);
		...
		if ( version_compare( $woocommerce_data['Version'], WC_WALLEE_REQUIRED_WC_MAXIMUM_VERSION, '>' ) ) {
			...
			if ( ! WC_Admin_Notices::user_has_dismissed_notice( $notice_id ) ) {
				$message = sprintf(
					...
					__( 'The plugin Wallee has been tested up to WooCommerce %1$s but you have installed the version %2$s. Please notice that this is not recommended.', 'woo-wallee' ),
					WC_WALLEE_REQUIRED_WC_MAXIMUM_VERSION,
	...
	public static function update_1_0_8_store_default_status_mappings() {
		$status_adapter = new WC_Wallee_Order_Status_Adapter();
