...
class WC_Wallee_Helper
{
    ...
    public function lock_by_transaction_id($space_id, $transaction_id)
    {
        ...
        $wpdb->query($wpdb->prepare("SELECT * FROM " . $wpdb->prefix . "woocommerce_wallee_transaction_info WHERE transaction_id = %d and space_id = %d FOR UPDATE", $transaction_id, $space_id));
        
        $wpdb->update($wpdb->prefix . 'woocommerce_wallee_transaction_info', $data_array, array(
            'transaction_id' => $transaction_id,
            'space_id' => $space_id
        ), $type_array, array(
            "%d",
        ...
        if (strlen($languageString) >= 5) {
            ...
            if (!$language && strpos($languageString, '-') !== false) {
                ...
                while (!$language && !empty($languageParts) ){
                    $language = WC_Wallee_Provider_Language::instance()->find(implode('-', $languageParts));
                    array_pop($languageParts);
                }
            }
        }
        if (! $language) {
            $language = WC_Wallee_Provider_Language::instance()->find_by_iso_code(strtolower(substr($languageString, 0, strpos($languageString, '-'))));
        }
    ...
    {
        if ('yes' === get_option('woocommerce_manage_stock') && $order && apply_filters('wc_wallee_can_increase_order_stock', true, $order) && sizeof($order->get_items()) > 0) {
