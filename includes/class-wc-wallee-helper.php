...
class WC_Wallee_Helper {
	...
	public function get_total_amount_including_tax( array $line_items, bool $excludeDiscounts = false) {
		...
		foreach ( $line_items as $line_item ) {
			...
			if ( $excludeDiscounts && $type === \Wallee\Sdk\Model\LineItemType::DISCOUNT && strpos( $name, WC_Wallee_Packages_Coupon_Discount::COUPON ) !== false ) {
	...
	public function cleanup_line_items( array $line_items, $expected_sum, $currency ) {
		...
		$has_coupons = apply_filters( 'wc_wallee_packages_coupon_cart_has_coupon_discounts_applied', $currency );
		...
		if ( $has_coupons ) {
			$result = apply_filters( 'wc_wallee_packages_coupon_process_line_items_with_coupons', $line_items, $expected_sum, $currency );
	...
	public function lock_by_transaction_id( $space_id, $transaction_id ) {
		...
		$wpdb->query( $wpdb->prepare( 'SELECT * FROM ' . $wpdb->prefix . 'wc_wallee_transaction_info WHERE transaction_id = %d and space_id = %d FOR UPDATE', $transaction_id, $space_id ) );

		$wpdb->update(
			$wpdb->prefix . 'wc_wallee_transaction_info',
			$data_array,
			array(
	...
	protected function restock_items_for_order( WC_Order $order ) {
		if ( 'yes' === get_option( 'woocommerce_manage_stock' ) && $order && apply_filters( 'wc_wallee_can_increase_order_stock', true, $order ) && count( $order->get_items() ) > 0 ) {
			foreach ( $order->get_items() as $item ) {
				...
				if ( $item->is_type( 'line_item' ) && $product && $product->managing_stock() ) {
					...
					if ( ! is_wp_error( $new_stock ) ) {
						...
						$order->add_order_note( sprintf( __( '%1$s stock increased from %2$s to %3$s.', 'woo-wallee' ), $item_name, $new_stock - $qty, $new_stock ) );
					}
				}
			}
