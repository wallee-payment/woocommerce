...
class WC_Wallee_Helper {
	...
	public static function is_custom_status_mapping_enabled() {
		$enabled = get_option( WooCommerce_Wallee::WALLEE_CK_ENABLE_CUSTOM_STATUS_MAPPING, 'no' );
		return 'yes' === apply_filters( 'wallee_enable_custom_status_mapping_value', $enabled );
	}
	...
	public static function map_status_to_current_mode( $status_slug, $prefixed = false ) {
		...
		$fallback_statuses = array(
			'wallee-manual' => 'on-hold',
			'wallee-redirected' => 'processing',
			'wallee-waiting' => 'processing',
		);
	...
	public static function update_order_status_for_preorder_if_needed( $order ) {
		...
		if ( $preorder_status_slug ) {
			...
			$order->update_status(
			  ...
			  __( 'Product is on pre-order. Status set automatically.', 'woo-wallee' )
			);

			$order->add_order_note(
			  __( 'Order status automatically set to pre-order.', 'woo-wallee' )
			);
		}
	}

	/**
	 * Get default Wallee settings array
	 *
	 * @return array
	 */
