		...
		if ( $gateway instanceof WC_Wallee_Gateway ) {
			...
			if ( ! in_array( $transaction_info->get_state(), self::$refundable_states ) ) {
				...
				if ( $existing_refund_job->get_id() > 0 ) {
					echo '<span class="wallee-action-in-progress">' . esc_html( __( 'There is a refund in progress.', 'woo-wallee' ) ) . '</span>';
					echo '<button type="button" class="button wallee-update-order">' . esc_html( __( 'Update', 'woo-wallee' ) ) . '</button>';
					echo '<span id="wallee-remove-refund" style="display:none;"></span>';
				}
	...
	public static function update_refunds() {
		...
		foreach ( $to_process as $id ) {
			try {
				...
				$message = sprintf( __( 'Error updating refund job with id %1$d: %2$s', 'woo-wallee' ), $id, $e->getMessage() );
