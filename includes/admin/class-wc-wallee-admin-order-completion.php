...
class WC_Wallee_Admin_Order_Completion {
	...
	public static function execute_completion() {
		...
		try {
			...
			$completion_job->set_state( WC_Wallee_Entity_Completion_Job::WALLEE_STATE_CREATED );
		...
		try {
			...
			WC()->session->set( 'wallee_order_id', $order_id );
	...
	protected static function update_line_items( $completion_job_id ) {
		...
		if ( $completion_job->get_state() !== WC_Wallee_Entity_Completion_Job::WALLEE_STATE_CREATED ) {
		...
		try {
			...
			$completion_job->set_state( WC_Wallee_Entity_Completion_Job::WALLEE_STATE_ITEMS_UPDATED );
			$completion_job->save();
			WC_Wallee_Helper::instance()->commit_database_transaction();
	...
	protected static function send_completion( $completion_job_id ) {
		...
		if ( $completion_job->get_state() !== WC_Wallee_Entity_Completion_Job::WALLEE_STATE_ITEMS_UPDATED ) {
		...
		try {
			...
			$completion_job->set_state( WC_Wallee_Entity_Completion_Job::WALLEE_STATE_SENT );
			$completion_job->save();
			WC_Wallee_Helper::instance()->commit_database_transaction();
		} catch ( \Wallee\Sdk\ApiException $e ) {
			if ( $e->getResponseObject() instanceof \Wallee\Sdk\Model\ClientError ) {
	...
	public static function update_for_order( WC_Order $order ) {
		...
		} elseif ( $completion_job->get_state() === WC_Wallee_Entity_Completion_Job::WALLEE_STATE_ITEMS_UPDATED ) {
			self::send_completion( $completion_job->get_id() );
		}
	}
