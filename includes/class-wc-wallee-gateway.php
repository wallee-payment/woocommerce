...
class WC_Wallee_Gateway extends WC_Payment_Gateway {
	...
	public function init_form_fields() {
		$this->form_fields = array(
			'enabled' => array(
				'title' => esc_html__( 'Enable/Disable', 'woo-wallee' ),
				'type' => 'checkbox',
			...
			'title_value' => array(
				'title' => esc_html__( 'Title', 'woo-wallee' ),
				'type' => 'info',
			...
			'description_value' => array(
				'title' => esc_html__( 'Description', 'woo-wallee' ),
				'type' => 'info',
	...
	public function is_available() {
		...
		try {
			...
			$gateway_available = WC()->session && WC()->session->has_session()
				? WC()->session->get( 'wallee_payment_gateways' )
				: array();
			...
			if ( apply_filters(
				...
				is_admin() || ! is_checkout() || ( isset( $GLOBALS['_wallee_calculating'] ) && $GLOBALS['_wallee_calculating'] ),
				$this
			) ) {
			...
			if ( WC()->session && WC()->session->has_session() ) {
				...
				WC()->session->set( 'wallee_payment_gateways', $gateway_available );
			}
	...
	protected function get_safe_possible_payment_methods_for_cart() {
		try {
			return WC_Wallee_Service_Transaction::instance()->get_possible_payment_methods_for_cart();
	...
	public function enqueue_checkout_scripts() {
		...
		if ( ! wp_script_is( 'wallee-remote-checkout-js', 'enqueued' ) ) {
            ...
            if (get_option( WooCommerce_Wallee::WALLEE_CK_INTEGRATION ) == WC_Wallee_Integration::WALLEE_PAYMENTPAGE) {
                return false;
			}
	...
	public function process_payment( $order_id ) {
		...
		try {
			...
			if ( $no_iframe || apply_filters( 'wc_wallee_gateway_result_send_json', $is_order_pay_endpoint, $order_id ) ) { //phpcs:ignore
				$result = array(
					'result' => 'success',
					'redirect' => $redirect_url,
				);
			...
			apply_filters( 'wallee_order_update_status', $order, 'failed' );
			$result = array(
				'result' => 'failure',
				'reload' => 'true',
			);
	...
	public function process_payment_transaction( $order, $transaction_id, $space_id, $is_order_pay_endpoint, $transaction_service ) {
		try {
			$transaction_service->api_client->addDefaultHeader(
				...
				WC_Wallee_Helper::WALLEE_CHECKOUT_TYPE_LEGACY
			);
