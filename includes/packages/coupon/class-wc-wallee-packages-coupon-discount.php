...
/**
 * WC_Wallee_Packages_Coupon_Discount Class
 *
 * Wallee
...
class WC_Wallee_Packages_Coupon_Discount {
	...
	public static function init() {
		...
		add_filter(
			'wc_wallee_packages_coupon_discount_totals_including_tax',
			array(
				__CLASS__,
		...
		add_filter(
			'wc_wallee_packages_coupon_has_coupon_discounts_applied',
			array(
				__CLASS__,
	...
	public static function copy_total_coupon_discount_meta_data_to_order_item( WC_Order_Item_Product $item, $cart_item_key, $values, WC_Order $order = null ) {
		$coupon_discount = apply_filters('wc_wallee_packages_coupon_percentage_discounts_by_item', $cart_item_key );
	...
	public static function get_coupons_discount_totals_including_tax( $currency ) {
		...
		$order_id = WC()->session->get( 'wallee_order_id' );
	...
	public static function has_coupon_discounts_applied( $currency ) {
		$discount = apply_filters( 'wc_wallee_packages_coupon_discount_totals_including_tax', $currency );
		return $discount > 0;
	}
	...
	public static function get_coupon_percentage_discount_by_item( string $item_key ) {
		$coupon_discounts_applied = apply_filters( 'wc_wallee_packages_coupon_discounts_applied_by_cart', WC()->cart );
	...
	public static function process_line_items_with_coupons( array $line_items, float $expected_sum, string $currency ) {
		...
		/** @var \Wallee\Sdk\Model\LineItemCreate $line_item */
		...
		if ( !self::compare_numbers( $expected_sum, $amount_rounded ) ) {
			...
			$line_item->setName( __( 'Coupon adjustment', 'woo-wallee' ) );
