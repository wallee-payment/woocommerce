...
class WC_Wallee_Service_Transaction extends WC_Wallee_Service_Abstract {
	...
	public function get_possible_payment_methods_for_cart(){
	    ...
	    if (!isset(self::$possible_payment_method_cache[$current_cart_id]) || is_null(self::$possible_payment_method_cache[$current_cart_id])) {
	        try {
				...
				$integration_method = get_option(WooCommerce_Wallee::CK_INTEGRATION);
	...
	protected function getOrderReference($order) {
		$reference_type = get_option(WooCommerce_Wallee::CK_ORDER_REFERENCE);

		if ($reference_type == WC_Wallee_Order_Reference::ORDER_NUMBER) {
	...
	public function get_transaction_from_session(){
		...
		if (!isset(self::$transaction_cache[$current_cart_id]) || self::$transaction_cache[$current_cart_id] == null) {
			...
			$configured_space_id = get_option(WooCommerce_Wallee::CK_SPACE_ID);
	...
	protected function create_transaction_by_order(WC_Order $order){
	    ...
	    if(isset($_COOKIE['wc_wallee_device_id'])){
	        $create_transaction->setDeviceSessionIdentifier(sanitize_text_field($_COOKIE['wc_wallee_device_id']));
	    }
    ...
    /**
     * @param WC_Order                                         $order
     * @param WC_Wallee_Entity_Transaction_Info $existing_transaction
	...
	protected function load_and_update_transaction_from_session(){
		...
		for ($i = 0; $i < 5; $i++) {
			try {
				...
				if ($transaction->getState() != \Wallee\Sdk\Model\TransactionState::PENDING || (!empty($transaction->getCustomerId()) && $transaction->getCustomerId() != $this->get_customer_id())) {
