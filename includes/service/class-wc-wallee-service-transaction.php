...
class WC_Wallee_Service_Transaction extends WC_Wallee_Service_Abstract {
	...
	{
		$this->api_client = WC_Wallee_Helper::instance()->get_api_client();
	}
	...
	protected function get_transaction_service() {
		if ( is_null( $this->transaction_service ) ) {
			$this->transaction_service = new \Wallee\Sdk\Service\TransactionService( $this->api_client );
		}
	...
	protected function get_transaction_iframe_service() {
		if ( is_null( $this->transaction_iframe_service ) ) {
			$this->transaction_iframe_service = new \Wallee\Sdk\Service\TransactionIframeService( $this->api_client );
		}
	...
	protected function get_transaction_lightbox_service() {
		if ( is_null( $this->transaction_lightbox_service ) ) {
			$this->transaction_lightbox_service = new \Wallee\Sdk\Service\TransactionLightboxService( $this->api_client );
		}
	...
	protected function get_transaction_payment_page_service() {
		if ( is_null( $this->transaction_payment_page_service ) ) {
			$this->transaction_payment_page_service = new \Wallee\Sdk\Service\TransactionPaymentPageService( $this->api_client );
		}
	...
	protected function get_charge_attempt_service() {
		if ( is_null( $this->charge_attempt_service ) ) {
			$this->charge_attempt_service = new \Wallee\Sdk\Service\ChargeAttemptService( $this->api_client );
		}
	...
	protected function get_transaction_line_item_version_service() {
		if ( is_null( $this->transaction_line_item_version_service ) ) {
			$this->transaction_line_item_version_service = new \Wallee\Sdk\Service\TransactionLineItemVersionService( $this->api_client );
		}
	...
	public function get_possible_payment_methods_for_cart() {
		...
		$paymentConfigurations[] = WC_Wallee_Zero_Gateway::ZERO_PAYMENT_CONF_ID;
	 ...
	 */
	protected function assemble_order_transaction_data( WC_Order $order, \Wallee\Sdk\Model\AbstractTransactionPending $transaction, bool $set_line_items = true ) {
	...
	protected function get_session_email_address() {
		...
		$sanitized_post_data = wp_verify_nonce( isset( $_POST['post_data'] ), 'wallee_nonce_block' );
	...
	protected function set_modified_order_line_items( WC_Order $order, $order_total, \Wallee\Sdk\Model\AbstractTransactionPending $transaction ) {
		$transaction->setLineItems( WC_Wallee_Service_Line_Item::instance()->get_items_for_renewal( $order, $order_total, $transaction ) );
	}
	...
	public function create_transaction_by_renewal_order( WC_Order $order, $order_total, $token_id ) {
		...
		WC_Wallee_Helper::instance()->add_headers( $this->api_client, [
			WC_Wallee_Helper::SUBSCRIPTION_TRANSACTION => true
		]);
		$transaction = $this->get_transaction_service()->create( $space_id, $create_transaction );
		$this->update_transaction_info( $transaction, $order );
		return $transaction;
	...
	public function store_performance_metric( $metric, $timestamp, $transaction_id ) {
		...
		$metrics = $session->get( 'wallee_performance_metrics', array() );
		$metrics[] = array(
			'metric' => $metric,
			'timestamp' => $timestamp,
			'transaction_id' => $transaction_id,
		);
		$session->set( 'wallee_performance_metrics', $metrics );
	}

	/**
	 * Adds performance metrics headers to the API client.
	 *
	 * @param \Wallee\Sdk\ApiClient $api_client The API client.
	 */
	private function add_performance_metrics_headers( \Wallee\Sdk\ApiClient $api_client ) {
		$session = WC()->session;
		if ( ! $session ) {
			return;
		}
