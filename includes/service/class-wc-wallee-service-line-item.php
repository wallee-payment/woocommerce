...
class WC_Wallee_Service_Line_Item extends WC_Wallee_Service_Abstract {
	...
	private function translate_item_language( $items ) {
		...
		if ( isset( $GLOBALS['TRP_LANGUAGE'] ) ) {
			$wallee_trp_language = $GLOBALS['TRP_LANGUAGE'];
		}
	...
	protected function create_product_line_items_from_session( WC_Cart $cart, $currency ) {
		...
		foreach ( $cart->get_cart() as $values ) {
			...
			$items[] = apply_filters( 'wc_wallee_modify_line_item_product_session', $this->clean_line_item( $line_item ), $values ); //phpcs:ignore
		}
		return $items;
	}
	...
	protected function create_fee_lines_items_from_session( WC_Cart $cart, $currency ) {
		...
		foreach ( $cart->get_fees() as $fee ) {
			...
			$fees[] = apply_filters( 'wc_wallee_modify_line_item_fee_session', $this->clean_line_item( $line_item ), $fee ); //phpcs:ignore
		}
		return $fees;
	}
	...
	protected function create_shipping_line_items_from_session( $packages, $chosen_methods, $currency ) {
		...
		foreach ( $packages as $package_key => $package ) {
			if ( isset( $chosen_methods[ $package_key ], $package['rates'][ $chosen_methods[ $package_key ] ] ) ) {
				...
				$shippings[] = apply_filters( 'wc_wallee_modify_line_item_shipping_session', $this->clean_line_item( $line_item ), $shipping_rate ); //phpcs:ignore
			}
		}
		return $shippings;
	}
	 ...
	 * @return \Wallee\Sdk\Model\LineItemCreate|null The line item created or null if the coupon is not valid.
	 */
	private function create_coupon_line_items( $coupon, float $total_discount_amount = 0, array $items = [] ) {
		...
		foreach ( $discounts as $discount ) {
			...
			$line_item->setName( sprintf( '%s: %s (%s%% tax)', WC_Wallee_Packages_Coupon_Discount::WALLEE_COUPON, $coupon->get_code(), $discount['rate_id'] ) );
			$line_item->setQuantity( 1 );
			$line_item->setShippingRequired( false );
			$line_item->setSku( $sku, 200 );
			$line_item->setType( \Wallee\Sdk\Model\LineItemType::DISCOUNT );
			...
			$tax_rate = new \Wallee\Sdk\Model\TaxCreate(
				array(
	...
	public function get_items_from_order( WC_Order $order ) {
		...
		$items = apply_filters( 'wc_wallee_modify_line_item_order', $raw, $order ); //phpcs:ignore
		$total = apply_filters( 'wc_wallee_modify_total_to_check_order', $order->get_total(), $order ); //phpcs:ignore
	...
	protected function create_product_line_items_from_order( WC_Order $order ) {
		...
		foreach ( $order_items as $item ) {
			...
			$items[] = apply_filters( 'wc_wallee_modify_line_item_product_order', $this->clean_line_item( $line_item ), $item ); //phpcs:ignore
		}
		return $items;
	}
	...
	protected function create_fee_lines_items_from_order( WC_Order $order ) {
		...
		foreach ( $order->get_fees() as $fee ) {
			...
			$fees[] = apply_filters( 'wc_wallee_modify_line_item_fee_order', $this->clean_line_item( $line_item ), $fee ); //phpcs:ignore
		}
		return $fees;
	}
	...
	protected function create_shipping_line_items_from_order( WC_Order $order ) {
		...
		foreach ( $order->get_shipping_methods() as $shipping ) {
			...
			$shippings[] = apply_filters( 'wc_wallee_modify_line_item_shipping_order', $this->clean_line_item( $line_item ), $shipping ); //phpcs:ignore
		}
		return $shippings;
	}
	...
	protected function create_product_line_items_from_backend( array $backend_items, WC_Order $order ) {
		...
		foreach ( $order->get_items() as $item_id => $item ) {
			...
			$item_data_coupon = $item->get_meta( '_wallee_coupon_discount_line_item_discounts' );
			...
			$items[] = apply_filters( 'wc_wallee_modify_line_item_product_backend', $this->clean_line_item( $line_item ), $item ); //phpcs:ignore
		}
		return $items;
	}
	...
	protected function create_fee_lines_items_from_backend( array $backend_items, WC_Order $order ) {
		...
		foreach ( $order->get_fees() as $fee_id => $fee ) {
			...
			$fees[] = apply_filters( 'wc_wallee_modify_line_item_fee_backend', $this->clean_line_item( $line_item ), $fee ); //phpcs:ignore
		}
		return $fees;
	}
	...
	protected function create_shipping_line_items_from_backend( array $backend_items, WC_Order $order ) {
		...
		foreach ( $order->get_shipping_methods() as $shipping_id => $shipping ) {
			...
			$shippings[] = apply_filters( 'wc_wallee_modify_line_item_shipping_backend', $this->clean_line_item( $line_item ), $shipping ); //phpcs:ignore
		}
		return $shippings;
	}
	 ...
	 * @throws WC_Wallee_Exception_Invalid_Transaction_Amount WC_Wallee_Exception_Invalid_Transaction_Amount.
	 */
	public function get_items_for_renewal( WC_Order $order, $order_total, \Wallee\Sdk\Model\AbstractTransactionPending $transaction ) {
		$raw_items = $this->get_raw_items_from_order( $order );
		return WC_Wallee_Helper::instance()->cleanup_line_items( $raw_items, $order_total, $order->get_currency(), true );
	}
