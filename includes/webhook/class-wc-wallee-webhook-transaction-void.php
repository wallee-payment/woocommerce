...
class WC_Wallee_Webhook_Transaction_Void extends WC_Wallee_Webhook_Order_Related_Abstract {
	 ...
	 * Canonical processor.
	 *
	 * @var WC_Wallee_Webhook_Transaction_Void_Strategy
	 */
	private $strategy;
	...
	public function __construct() {
		$this->strategy = new WC_Wallee_Webhook_Transaction_Void_Strategy();
	}
	...
	protected function load_entity( WC_Wallee_Webhook_Request $request ) {
		wc_deprecated_function(
            __METHOD__,
            '3.0.12',
            'WC_Wallee_Webhook_Transaction_Void_Strategy::load_entity'
        );
	...
	protected function get_order_id( $void_transaction ) {
		wc_deprecated_function(
            __METHOD__,
            '3.0.12',
            'WC_Wallee_Webhook_Transaction_Void_Strategy::get_order_id'
        );
	...
	protected function process_order_related_inner( WC_Order $order, $void_transaction, $request ) {
		wc_deprecated_function(
            __METHOD__,
            '3.0.12',
            'WC_Wallee_Webhook_Transaction_Void_Strategy::process_order_related_inner'
        );
