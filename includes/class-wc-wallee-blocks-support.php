 ...
 */
final class WC_Wallee_Blocks_Support extends AbstractPaymentMethodType {
	/**
	 * Payment method name defined by payment methods extending this class.
	 *
	 * @var string
	 */
	protected $name = 'wallee';

	/**
	 * Initializes the payment method type.
	 */
	public function initialize() {
		$this->settings['space_id'] = get_option( WooCommerce_Wallee::WALLEE_CK_SPACE_ID );
	}
	...
	public function get_payment_method_script_handles() {
		...
		wp_register_script(
			...
			WooCommerce_Wallee::instance()->plugin_url() . '/assets/js/frontend/blocks/build/index.js',
			$dependencies,
			$version,
			true
		);
		...
		wp_localize_script(
			...
			'wallee_block_params',
			array(
				'wallee_nonce' => wp_create_nonce( 'wallee_nonce_block' ),
				'i18n' => array(
					'loading' => __( 'Loading payment method...', 'woo-wallee' ),
					'load_error' => __( 'Unable to load this payment method. Please refresh the page and try again.', 'woo-wallee' ),
				),
			)
		);

		return array( 'WooCommerce_Wallee_blocks_support' );
	}
	...
	public function get_payment_method_data() {
		return array(
			'title' => 'Wallee',
			'description' => 'Wallee description',
		);
	}
	...
	public static function get_payment_methods(): array {
		try {
			...
			$payment_plugin = array_filter(
			  $payment_gateways,
			  fn( $key ) => str_contains( $key, 'wallee_' ),
			...
			$payments_list =
			  array_map(
				function ( $payment_gateway ) use ( $available_payment_methods ) {
					...
					$integration_mode = $payment_gateway->get_payment_configuration_id() === WC_Wallee_Zero_Gateway::ZERO_PAYMENT_CONF_ID
						? WC_Wallee_Integration::WALLEE_PAYMENTPAGE
						: get_option( WooCommerce_Wallee::WALLEE_CK_INTEGRATION );
	...
	public static function get_payment_methods_json() {
		if ( ! isset( $_POST['wallee_nonce'] ) || ! wp_verify_nonce( sanitize_key( wp_unslash( $_POST['wallee_nonce'] ) ), 'wallee_nonce_block' ) ) {  //phpcs:ignore
	...
	public static function process_payment( PaymentContext $context, PaymentResult &$result ) {
		...
		$transaction_service->api_client->addDefaultHeader(
			WC_Wallee_Helper::WALLEE_CHECKOUT_VERSION,
			WC_Wallee_Helper::WALLEE_CHECKOUT_TYPE_BLOCKS
		);
		...
		if ( WC_Wallee_Integration::WALLEE_PAYMENTPAGE === $integration_mode ) {
			$transaction_service->update_transaction_info( $transaction, $context->order );
			$redirect_url = $transaction_service->get_payment_page_url( get_option( WooCommerce_Wallee::WALLEE_CK_SPACE_ID ), $transaction->getId() );
		}
