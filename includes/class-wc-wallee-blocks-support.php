...
final class WC_Wallee_Blocks_Support extends AbstractPaymentMethodType {
	...
	public static function get_payment_methods(): array {
		...
		$payment_plugin = array_filter( $payment_gateways, fn( $key ) => str_contains( $key, 'wallee_' ), ARRAY_FILTER_USE_KEY );

		// Build the list with the keys expected by WooCommerce Blocks registering functionality.
		$payments_list = array_map(
			function ( WC_Wallee_Gateway $payment_gateway ) {
				return array(
					...
					'integration_mode' => ( get_option( WooCommerce_Wallee::WALLEE_CK_INTEGRATION ) == WC_Wallee_Integration::WALLEE_LIGHTBOX ) ? 'lightbox' : 'iframe',
	...
	public static function enqueue_portal_scripts() {
		...
		try {
			...
			if ( get_option( WooCommerce_Wallee::WALLEE_CK_INTEGRATION ) == WC_Wallee_Integration::WALLEE_LIGHTBOX ) {
