...
class WC_Wallee_Email {
	...
	public static function send_on_hold_email_when_authorized( $order_id ) {
		...
		self::send_on_hold_email( $order, '_wallee_on_hold_email_sent', true );
	}
	...
	private static function get_transaction_mapped_status( string $transaction_status ) {
		$status = apply_filters( 'wallee_wc_status_for_transaction', $transaction_status );
		if ( empty( $status ) ) {
			$defaults = apply_filters( 'wallee_default_order_status_mappings', array() );
			...
			add_filter(
				...
				function ( $actions ) use ( $manual_status, $processing_status ) {
					...
					if ( $manual_status && $processing_status ) {
						WC_Wallee_Email::add_unique_action( $all, 'woocommerce_order_status_' . $processing_status . '_to_' . $manual_status );
						WC_Wallee_Email::add_unique_action( $all, 'woocommerce_order_status_' . $processing_status . '_to_' . $manual_status . '_notification' );
						WC_Wallee_Email::add_unique_action( $all, 'woocommerce_order_status_' . $manual_status . '_to_' . $processing_status );
						WC_Wallee_Email::add_unique_action( $all, 'woocommerce_order_status_' . $manual_status . '_to_' . $processing_status . '_notification' );

						WC_Wallee_Email::add_unique_action( $customers, 'woocommerce_order_status_' . $processing_status . '_to_' . $manual_status . '_notification' );
						WC_Wallee_Email::add_unique_action( $customers, 'woocommerce_order_status_' . $manual_status . '_to_' . $processing_status . '_notification' );
					}
     ...
     * when the order was placed using any Wallee payment method
    ...
    public static function disable_pending_payment_email( $enabled, $order ) {
        ...
        $disable = get_option( WooCommerce_Wallee::WALLEE_CK_DISABLE_PENDING_EMAIL, 'no' );
        $gateway = wc_get_payment_gateway_by_order( $order );

        if ( $gateway instanceof WC_Wallee_Gateway && $disable === 'yes' ) {
            return false;
        }
