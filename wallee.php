...
if ( ! class_exists( 'WooCommerce_Wallee' ) ) {
	...
	final class WooCommerce_Wallee {
		...
		protected function init_hooks() {
			...
			add_action(
				'wp_ajax_wallee_custom_order_status_save_changes',
				array(
					$this,
			...
			add_action(
				'wp_ajax_wallee_custom_order_status_delete',
				array(
					$this,
		...
		public function loaded() {
			...
			add_filter(
			 	...
			 	function($value, $order) {
					...
					if (substr_count($order_data['payment_method'], "wallee")) {
	...
	public function valid_order_statuses_for_payment( $statuses, $order = null ) { //phpcs:ignore
		...
		public function custom_order_status_save_changes() {
			...
			if ( ! isset( $_POST['wallee_order_statuses_nonce'] ) || ! wp_verify_nonce( sanitize_key( wp_unslash( $_POST['wallee_order_statuses_nonce'] ) ), 'wallee_order_statuses_nonce' ) ) {  //phpcs:ignore
			...
			if ( isset( $data['key'] ) ) {
				...
				$sanitized_key_underscore = str_replace( [' ', '-'], '_', WC_Wallee_Order_Status_Adapter::WALLEE_CUSTOM_ORDER_STATUS_PREFIX . $first_key );
		...
		public function custom_order_status_delete() {
			...
			// Verify nonce.
			if ( ! isset( $_POST['wallee_order_statuses_nonce'] ) || ! wp_verify_nonce( sanitize_key( wp_unslash( $_POST['wallee_order_statuses_nonce'] ) ), 'wallee_order_statuses_nonce' ) ) {
				wp_send_json_error( 'Invalid request: missing nonce value', 403 );
			}
			...
			// Check if this order status is being used in wp_options with key prefix "wallee_order_status_mapping_".
			$like_pattern = WC_Wallee_Order_Status_Adapter::WALLEE_ORDER_STATUS_MAPPING_PREFIX . '%';
			...
			if ( $is_used > 0 ) {
				...
				wp_send_json_error( ['message' => __( 'Cannot delete: this status is currently in use.', 'woo-wallee' ), 'key' => ucfirst( $sanitized_key )], 400 );
			}

			 // Get the custom order status in wp_options with key prefix "wallee_order_status_mapping_".
			$like_pattern = WC_Wallee_Order_Status_Adapter::WALLEE_CUSTOM_ORDER_STATUS_PREFIX . '%';
			...
			wp_send_json_success( ['message' => __( 'Deleted successfully', 'woo-wallee' ), 'key' => ucfirst( $sanitized_key )] );
		}
		...
		public function before_calculate_totals( $cart ) { //phpcs:ignore
			$GLOBALS['wallee_calculating'] = true;
		}
		...
		public function after_calculate_totals( $cart ) { //phpcs:ignore
			unset( $GLOBALS['wallee_calculating'] );
		}
		...
		public function add_gateways( $methods ) {
			...
			require_once __DIR__ . '/includes/class-wc-wallee-zero-gateway.php';
			$methods[] = 'WC_Wallee_Zero_Gateway';
		...
		public function woocommerce_attribute_added( $attribute_id, $data ) { //phpcs:ignore
			...
			$nonce = isset( $_POST['wallee_attribute_option_send_nonce'] )
				? sanitize_text_field( wp_unslash( $_POST['wallee_attribute_option_send_nonce'] ) )
				: '';
			if ( ! $nonce || ! wp_verify_nonce( $nonce, 'wallee_attribute_option_send' ) ) {
				return;
			}
			$send = ! empty( $_POST['wallee_attribute_option_send'] ) ? 1 : 0;
