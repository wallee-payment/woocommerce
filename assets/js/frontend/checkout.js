...
/* global jQuery document window wallee_js_params */
...
jQuery(
	function ($) {
		var wc_wallee_checkout = {
			 ...
			 */
			info : function () {
				var versions = window.wallee_js_params.versions || {};
				var info = [
					{library: 'integration', version: window.wallee_js_params.integration},
					...
					{library: 'woo-wallee', version: versions.wallee || null},
				];
				console.table( info, [] );
			},
			...
			init : function () {
				...
				this.$checkout_form.off( 'button#place_order' ).on(
					...
					{ self : this },
					function (event) {
						if (wallee_js_params.integration === window.wc_wallee_checkout.integrations.LIGHTBOX ) {
							return false;
						}
					}
				);
				...
				if ($( document.body ).hasClass( 'woocommerce-order-pay' )) {
					...
					this.$order_review.off( 'click.woo-wallee' ).on(
						'click.woo-wallee',
						'input[name="payment_method"]',
						{
							self : this
				...
				window.wc_wallee_checkout = this;
			},
			...
			payment_method_click : function (event) {
				...
				if ( wallee_js_params.integration === self.integrations.PAYMENT_PAGE ) {
					return;
				}
				...
				var configuration = $( "#wallee-method-configuration-" + current_method );
			...
			enable_place_order_button: function () {
				...
				this.$order_button.removeClass( 'wallee-disabled' );
			},

			disable_place_order_button: function () {
				this.$order_button.prop( 'disabled', true );
				this.$order_button.addClass( 'wallee-disabled' );
			},
			...
			is_supported_method : function (method_id) {
				if (!method_id || method_id === 'wallee_zero') {
					return false;
				}
			...
			register_method : function (method_id, configuration_id, container_id) {
				...
				if (this.checkout_form_identifier === '#order_review') {
					this.$checkout_form.off( 'submit.wallee' )
					.on(
						'submit.wallee',
						function () {
					...
					this.$checkout_form.off( 'checkout_place_order_' + method_id + '.wallee' )
					.on(
			...
			process_submit : function (method_id) {
				...
				if ( ! has_full_address) {
					...
					this.submit_error( wallee_js_params.i18n_not_complete );
					return false;
				}
						...
						$.ajax(
							{
								...
								success: function ( result ) {
									...
									if (typeof result.wallee == 'undefined') {
			...
			process_order_created : function (data, textStatus, jqXHR) {
				...
				if (wallee_js_params.integration && wallee_js_params.integration === self.integrations.LIGHTBOX ) {
					...
					if ( ! has_full_address) {
						...
						self.submit_error( wallee_js_params.i18n_not_complete );
						return false;
					}
