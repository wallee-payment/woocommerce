...
jQuery(function($) {
	...
	show_refund_states : function() {
	    $('tbody#order_refunds div.wallee-refund-status')
		    .each(
			    function() {
				...
				if (state == 'failure') {
				    refund_thumb
					    .addClass('wallee-refund-state-failue');
	...
	show_completion : function(event) {
	    ...
	    $('div.wc-order-add-item').children(
		    'button.wallee-completion-button').each(
		    function(key, value) {
			$(value).show();
	    ...
	    $('#woocommerce-order-items').find('input.line_total').each(
			...
			$(this).closest('td.line_cost').find(
				...
				'wallee-initial-amount', $(this).val());
	...
	show_void : function() {
	    ...
	    $('div.wc-order-add-item').children('button.wallee-void-button')
