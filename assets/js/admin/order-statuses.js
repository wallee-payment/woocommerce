/* global walleeOrderStatusesLocalizeScript, ajaxurl */
( function( $, data, wp, ajaxurl ) {
	$( function() {
		var $tbody = $('.wallee-order-statuses-rows'),
			$save_button = $('.wallee-order-status-save');
		...
		var $row_template = templatesExist('wallee-order-statuses-row') ? wp.template('wallee-order-statuses-row') : null,
			$blank_template = templatesExist('wallee-order-statuses-row-blank') ? wp.template('wallee-order-statuses-row-blank') : null;
		...
		var OrderStatus = Backbone.Model.extend({
			save: function( changes ) {
				$.post( ajaxurl, {
					action : 'wallee_custom_order_status_save_changes',
					wallee_order_statuses_nonce : data.wallee_order_statuses_nonce,
					changes,
		...
		var OrderStatusView = Backbone.View.extend({
			...
			initialize: function() {
				...
				$( document.body ).on( 'click', '.wallee-order-status-add-new', { view: this }, this.configureNewOrderStatus );
			...
			initRow: function( rowData ) {
				...
				$tr.find( '.view' ).show();
				$tr.find( '.edit' ).hide();
				$tr.find( '.wallee-order-status-edit' ).on( 'click', { view: this }, this.onEditRow );
				$tr.find( '.wallee-order-status-delete' ).on( 'click', { view: this }, this.onDeleteRow );
			},
			configureNewOrderStatus: function( event ) {
				...
				$( this ).WCBackboneModal({
					template : 'wallee-order-statuses-configure',
					variable : {
						key,
						action: 'create',
					},
			...
			onConfigureOrderStatusSubmitted: function( event, target, posted_data ) {
				if ( target === 'wallee-order-statuses-configure' ) {
					const view = event.data.view;
					const model = view.model;
					const isNewRow = posted_data.key.includes( 'wallee_custom_order_status_' );
					const rowData = {
						...posted_data,
					};
			...
			onLoadBackboneModal: function( event, target ) {
				if ( 'wallee-order-statuses-configure' === target ) {
			...
			onDeleteRow: function( event ) {
				...
				$.post( ajaxurl, {
					action: 'wallee_custom_order_status_delete',
					wallee_order_statuses_nonce: data.wallee_order_statuses_nonce,
					key: key
				}, function( response ) {
					if ( response.success ) {
		...
		OrderStatusView.prototype.showErrorModal = function(message) {
			...
			var modalHtml = `
				<div id="wallee-error-modal" class="wc-backbone-modal wc-modal-shipping-zone-method-settings" style="max-width: 400px; margin: auto;">
			...
			$('#wallee-error-modal').WCBackboneModal();

			// Remove modal from DOM when closed
			$(document).on('click', '.wc-backbone-modal .modal-close', function() {
				$('#wallee-error-modal').remove();
			});
		};
	});
})( jQuery, walleeOrderStatusesLocalizeScript, wp, ajaxurl );
